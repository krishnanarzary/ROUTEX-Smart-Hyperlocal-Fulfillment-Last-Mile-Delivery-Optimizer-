...
state_rows = pd.read_csv(os.path.join(os.path.dirname(__file__), 'tat.csv'))
df1 = pd.read_csv(os.path.join(os.path.dirname(__file__), 'superstore.csv'))
df = pd.read_csv(os.path.join(os.path.dirname(__file__), 'state_reading.csv'))

model_path = os.path.join(os.path.dirname(__file__), 'state_prediction.joblib')
...
def plot_from_df(df, folium_map):
    ...
    for i, row in df.iterrows():
        ...
        icon_size = row.get('Icon_Size', 50)
        opacity = row.get('Opacity', 1)

        icon = folium.features.CustomIcon(IM_CONSTANTS.get(icon_id, IM_CONSTANTS[0]), icon_size=(icon_size, icon_size))
...
def load_df():
    path = os.path.join(os.path.dirname(__file__), 'data', 'walmart-stores.csv')
    if not os.path.exists(path):
        #st.error(f'File not found: {path}')
        return pd.DataFrame()
    walmartStores = pd.read_csv(path)
...
def load_Citydf():
    path = os.path.join(os.path.dirname(__file__), '..', 'data', 'cityData.csv')
    path = os.path.abspath(path)

    if not os.path.exists(path):
        st.error(f'City Data file not found: {path}')
        return pd.DataFrame()
...
def load_map():
    ...
    if df.empty:
        pass
        #st.warning("Walmart stores data is empty!")
    else:
        m = plot_from_df(df, m)
    return m
...
def main():
    ...
    st.markdown("""
    <style>
        /* Global background in PMS 2728 C */
        .main, .block-container {
            background-color: #0053E2 !important;  /* PMS 2728 C */
...
#         result = str(prediction[0]).strip()
    ...
    # st.code('Metrics')
            ...
            if st.button('Predict', key='same'):
                ...
                st.write('__________________________________________________________')
                st.title('YoY Transaction Trends')
